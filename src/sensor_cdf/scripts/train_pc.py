...
class GraphDataCollator:
    ...
    def __call__(self, batch):
        states, graphs, gradients, cdfs = [], [], [], []
        
        for batch_idx, (state, pc, grad, cdf) in enumerate(batch):
            pc = (pc - np.mean(pc, axis=0)) / (np.std(pc, axis=0) + 1e-8)
        ...
        return (
            ...
            torch.tensor(np.array(gradients), dtype=torch.float32),
            torch.tensor(np.array(cdfs), dtype=torch.float32)
        )

def load_data(dataset_path='/home/ubuntu/gxf/model/lidar_lcdf_1.npz'):
    ...
    return (states, pointclouds), (gradients, cdfs)
...
def train(model, train_loader, optimizer, criterion, saver, 
         task_type='grad', scheduler=None, num_epochs=10, 
    ...
    for epoch in range(num_epochs):
        ...
        for states, graph_batch, grads, cdfs in train_loader:
            states = states.to(device, non_blocking=True)
            graph_batch = graph_batch.to(device, non_blocking=True)

            with torch.amp.autocast(device_type='cuda', dtype=torch.float16):
                labels = grads.to(device) if task_type == 'grad' else cdfs.to(device).unsqueeze(1)
...
def main():
    ...
    # 数据加载
    (states, pointclouds), (gradients, cdfs) = load_data()
    dataset = list(zip(states, pointclouds, gradients, cdfs))
    ...
    # 模型初始化
    if config["task_type"] == 'grad':
        model = GradNet(hidden_dim=512).to(device)
    else:
        model = CDFNet(hidden_dim=512).to(device)
    ...
    criterion = nn.HuberLoss(delta=1.0) if config["task_type"] == 'grad' else nn.MSELoss()
    
    # 学习率调度
    scheduler = (
        OneCycleLR(optimizer, max_lr=config["learning_rate"]*3, total_steps=config["num_epochs"]*len(train_loader), pct_start=0.3)
        if config["task_type"] == 'grad' 
        else CosineAnnealingWarmRestarts(optimizer, T_0=100, T_mult=2, eta_min=1e-6)
    )

    # 模型保存设置
    save_path = (
        "/home/ubuntu/gxf/lidar_grad_model.pt" 
        if config["task_type"] == 'grad' 
        else "/home/ubuntu/gxf/lidar_cdf_model.pt"
    )
