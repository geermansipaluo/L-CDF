...
class ParametricEllipseTracker:
    ...
    def print_final_report(self):
        completed = max(1, self.completed_episodes)

        arrival_rate = 100.0 * self.reached_goals_count / completed
        success_rate = 100.0 * self.perfect_runs_count / completed
        avg_collisions = self.total_collision_events / completed

        print("\n" + " DensityNet 随机环境闭环评测报告 ".center(70, "="))
        print(f"完成回合数                 : {self.completed_episodes} / {self.total_eval_episodes}")
        print(f"到达率 Arrival Rate        : {arrival_rate:.2f}%  ({self.reached_goals_count}/{completed})")
        print(f"成功率 Success Rate        : {success_rate:.2f}%  ({self.perfect_runs_count}/{completed})")
        print(f"平均碰撞次数 Avg Collision : {avg_collisions:.2f} 次/回合")
        print(f"总碰撞触发次数             : {self.total_collision_events}")
        print("=" * 70 + "\n")

    # ---------------- 回合逻辑 ----------------
    def check_reach_goal(self, event):
        dist = np.linalg.norm(
            np.array(self.current_pose[:2]) - np.array(self.target_pos)
        )
        ...
        if reached:
            ...
            if not self.collision_happened_in_current_run:
                ...
                rospy.loginfo(
                    f"🏆 第 {self.completed_episodes} 回合成功：到达目标，且无碰撞"
                )
            else:
                rospy.logwarn(
                    f"⚠️ 第 {self.completed_episodes} 回合到达目标，但中途发生碰撞"
                )

        else:
            rospy.logwarn(
                f"⏰ 第 {self.completed_episodes} 回合超时失败，"
                f"当前距离目标 {dist:.3f} m"
            )
        ...
        # 判断是否完成全部评测
        if self.completed_episodes >= self.total_eval_episodes:
            rospy.logerr("🏁 评测完成：已跑满 10 回合，停止仿真节点")
            self.print_final_report()
            self.publish_twist(0.0, 0.0)
        ...
        try:
            ...
            rospy.logwarn(f"调用 /gazebo/reset_simulation 失败，尝试命令行 reset: {e}")
